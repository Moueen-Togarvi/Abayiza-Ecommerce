	...
	<div class="mb-6 flex items-center justify-between">
		...
		<div class="relative inline-block text-left">
			<select
				...
				class="block w-full rounded-md border border-[#000]/15 bg-white py-2 pr-10 pl-3 text-base text-[#000] shadow-sm focus:border-blue-500 focus:ring-blue-500 focus:outline-none sm:text-sm"
			>
				<option>Today</option>
					...
					<div class="ml-5 w-0 flex-1">
						<dl>
							...
							<dd class="flex items-baseline">
								<div class="text-2xl font-bold text-[#000]">
									{formatMoney(data.stats.totalSales)}
								</div>
							</dd>
						</dl>
					</div>
				...
				<div class="flex items-center">
					...
					<div class="ml-5 w-0 flex-1">
						<dl>
							...
							<dd class="flex items-baseline">
								<div class="text-2xl font-bold text-[#000]">
									{formatMoney(data.stats.averageOrderValue)}
								</div>
							</dd>
						</dl>
					</div>
					...
					{#each data.topProducts as product}
						<li class="p-4 hover:bg-blue-50/60">
							<div class="flex items-center space-x-4">
								...
								<div>
									<span
										class="inline-flex items-center rounded-full bg-yellow-100 px-2.5 py-0.5 text-xs font-medium text-yellow-900"
									>
				...
				<table class="min-w-full divide-y divide-[#000]/10">
					<thead class="bg-orange-50/50">
						<tr>
							<th
								scope="col"
								class="px-6 py-3 text-left text-xs font-medium tracking-wider text-[#000]/55 uppercase"
								>Order</th
							>
							<th
								scope="col"
							...
							<th
								scope="col"
								class="px-6 py-3 text-right text-xs font-medium tracking-wider text-[#000]/55 uppercase"
								>Total</th
							>
						</tr>
					</thead>
					<tbody class="divide-y divide-[#000]/10 bg-white">
						{#each data.recentOrders as order}
							<tr class="hover:bg-blue-50/50">
								...
								<td class="px-6 py-4 text-sm whitespace-nowrap text-[#000]/55"
									>{order.customerName}</td
								>
								<td class="px-6 py-4 whitespace-nowrap">
									<span
										class="inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium {order.status ===
										'PENDING'
								...
								<td class="px-6 py-4 text-right text-sm font-medium whitespace-nowrap text-[#000]"
					...
					{#each data.lowStockVariants as variant}
						<li class="flex items-center justify-between p-4 hover:bg-red-50/50">
							...
							<div class="text-right">
								...
								<p class="mt-1 cursor-pointer text-xs text-blue-600">Update stock</p>
							</div>
						</li>
					{/each}
					{#if data.lowStockVariants.length === 0}
						<li class="p-4 text-center text-sm text-[#000]/55">Inventory levels are healthy</li>
					{/if}
				</ul>
			</div>
		</div>
