<script lang="ts">
	...
	const displayName = (user: any) => `${user.firstName || ''} ${user.lastName || ''}`.trim() || 'Unnamed user';
...
<div class="mx-auto max-w-7xl pb-12">
	<div class="mb-6 flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
		<div>
			...
			<p class="mt-2 text-sm text-gray-500">Create users, assign roles, block access, and reset passwords.</p>
		</div>
		{#if canManage}
			<button
				type="button"
	...
	<form method="GET" class="rounded-t-2xl border-x border-t border-gray-200 bg-white p-4 shadow-sm">
		<div class="grid gap-3 md:grid-cols-[1fr_12rem_auto]">
			...
			/>
			<select name="role" class="rounded-md border-gray-300 text-sm focus:border-black focus:ring-black">
			...
			<div class="flex gap-2">
				<button type="submit" class="rounded-md bg-black px-5 py-2 text-sm font-bold text-white hover:bg-gray-800">Filter</button>
				<a href="/abayiza-secure-admin-7k9x2p/users" class="rounded-md border border-gray-300 px-5 py-2 text-sm font-bold text-gray-600 hover:bg-gray-50">Clear</a>
			</div>
		</div>
	</form>
	...
	<div class="overflow-x-auto rounded-b-2xl border border-gray-200 bg-white shadow-sm">
		<table class="min-w-full divide-y divide-gray-200">
			<thead class="bg-gray-50">
				<tr>
					<th class="px-6 py-3 text-left text-xs font-bold tracking-wider text-gray-500 uppercase">User</th>
					<th class="px-6 py-3 text-left text-xs font-bold tracking-wider text-gray-500 uppercase">Role</th>
					<th class="px-6 py-3 text-left text-xs font-bold tracking-wider text-gray-500 uppercase">Status</th>
					<th class="px-6 py-3 text-left text-xs font-bold tracking-wider text-gray-500 uppercase">Orders</th>
					<th class="px-6 py-3 text-left text-xs font-bold tracking-wider text-gray-500 uppercase">Password</th>
					<th class="px-6 py-3 text-right text-xs font-bold tracking-wider text-gray-500 uppercase">Actions</th>
				</tr>
			</thead>
						...
						<td class="px-6 py-4">
							<div class="font-bold text-gray-950">
								...
								{#if user.isCurrentUser}
									<span class="ml-2 rounded-full bg-green-100 px-2 py-0.5 text-[0.65rem] font-black text-green-700">You</span>
								{/if}
							</div>
							<div class="text-xs text-gray-500">{user.email}</div>
							<div class="mt-1 text-[0.7rem] text-gray-400">Joined {new Date(user.createdAt).toLocaleDateString()}</div>
						</td>
						<td class="px-6 py-4">
							<span class={`rounded-full px-2.5 py-1 text-xs font-black ${roleClass(user.role)}`}>{user.role}</span>
						</td>
						<td class="px-6 py-4">
							<span class={`rounded-full px-2.5 py-1 text-xs font-black ${user.isBlocked ? 'bg-red-100 text-red-700' : 'bg-green-100 text-green-700'}`}>
							...
							{#if canManage}
								<div class="flex justify-end gap-2">
									...
									<button
										...
										onclick={() => (pendingDeleteUser = { id: user.id, email: user.email, name: displayName(user) })}
									>
										Delete
									</button>
								</div>
								{#if user.isCurrentUser}
									<p class="mt-2 text-right text-xs text-gray-400">Current admin cannot be deleted.</p>
								{/if}
							{:else}
	...
	{#if isCreateModalOpen}
		<div class="fixed inset-0 z-50 flex items-center justify-center bg-black/45 px-4 py-6">
			<div class="max-h-[92vh] w-full max-w-2xl overflow-y-auto rounded-2xl border border-gray-200 bg-white p-6 shadow-2xl">
				<div class="mb-5 flex items-start justify-between gap-4">
					<div>
						...
						<p class="mt-1 text-sm text-gray-500">Create customer or admin access from one simple form.</p>
					</div>
					<button
						type="button"
				...
				<form method="POST" action="?/create" class="space-y-4">
					<div class="grid gap-4 sm:grid-cols-2">
						<label class="block">
							...
							<input name="firstName" placeholder="First name" class="w-full rounded-md border-gray-300 text-sm focus:border-black focus:ring-black" />
						</label>
						<label class="block">
							<span class="mb-1 block text-xs font-black text-gray-500 uppercase">Last Name</span>
							<input name="lastName" placeholder="Last name" class="w-full rounded-md border-gray-300 text-sm focus:border-black focus:ring-black" />
						</label>
					</div>
					<label class="block">
						<span class="mb-1 block text-xs font-black text-gray-500 uppercase">Email</span>
						<input name="email" type="email" required placeholder="email@example.com" class="w-full rounded-md border-gray-300 text-sm focus:border-black focus:ring-black" />
					</label>
					<div class="grid gap-4 sm:grid-cols-2">
						<label class="block">
							<span class="mb-1 block text-xs font-black text-gray-500 uppercase">Role</span>
							<select name="role" class="w-full rounded-md border-gray-300 text-sm focus:border-black focus:ring-black">
					...
					<label class="flex items-center gap-2 text-sm font-bold text-gray-700">
						<input name="isBlocked" type="checkbox" class="rounded border-gray-300 text-black focus:ring-black" />
						Blocked
					</label>
					<div class="flex justify-end gap-3 border-t border-gray-100 pt-4">
						...
						<button type="submit" class="rounded-full bg-black px-5 py-2 text-sm font-black text-white hover:bg-gray-800">
							Create User
						</button>
					</div>
				</form>
			...
			<div class="max-h-[92vh] w-full max-w-2xl overflow-y-auto rounded-2xl border border-gray-200 bg-white p-6 shadow-2xl">
				...
				<form method="POST" action="?/update" class="space-y-4">
					...
					<div class="grid gap-4 sm:grid-cols-2">
						<label class="block">
							<span class="mb-1 block text-xs font-black text-gray-500 uppercase">First Name</span>
							<input name="firstName" value={activeEditUser.firstName || ''} placeholder="First name" class="w-full rounded-md border-gray-300 text-sm focus:border-black focus:ring-black" />
						</label>
						<label class="block">
							<span class="mb-1 block text-xs font-black text-gray-500 uppercase">Last Name</span>
							<input name="lastName" value={activeEditUser.lastName || ''} placeholder="Last name" class="w-full rounded-md border-gray-300 text-sm focus:border-black focus:ring-black" />
						</label>
					</div>
					<div class="grid gap-4 sm:grid-cols-2">
						<label class="block">
							<span class="mb-1 block text-xs font-black text-gray-500 uppercase">Role</span>
							<select name="role" class="w-full rounded-md border-gray-300 text-sm focus:border-black focus:ring-black">
								<option value="CUSTOMER" selected={activeEditUser.role === 'CUSTOMER'}>Customer</option>
								<option value="EDITOR" selected={activeEditUser.role === 'EDITOR'}>Editor</option>
								<option value="SUPER_ADMIN" selected={activeEditUser.role === 'SUPER_ADMIN'}>Super admin</option>
							</select>
						</label>
						<label class="block">
							...
							<div class="relative">
								...
								<button
									type="button"
									aria-label={visiblePasswordFor[activeEditUser.id] ? 'Hide password' : 'Show password'}
					...
					<label class="flex items-center gap-2 text-sm font-bold text-gray-700">
						<input name="isBlocked" type="checkbox" checked={activeEditUser.isBlocked} class="rounded border-gray-300 text-black focus:ring-black" />
						Blocked
					</label>
					<div class="flex justify-end gap-3 border-t border-gray-100 pt-4">
						<button
			...
			<div class="w-full max-w-md rounded-2xl border border-gray-200 bg-white p-6 shadow-2xl">
				...
				<div class="mt-5 flex justify-end gap-3">
					...
					<form method="POST" action="?/delete">
						...
						<button type="submit" class="rounded-full bg-red-600 px-5 py-2 text-sm font-black text-white hover:bg-red-700">
							Delete User
						</button>
					</form>
				</div>
